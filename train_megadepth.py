...
cfg['epochs'] = 30
cfg["end_lr"] = 1e-7
cfg["warming_up_epochs"] = 3
cfg['milestones'] = [5,9,13,17,21]
cfg["weight_decay"] = 1e-2
cfg['weights_dir'] = 'weights/megadepth'
...
def train(local_rank,cfg):
    ...
    dataset_train,distributed_sampler = build_distributed_dataset(
        megadepth_root_dir, 
        megadepth_train_val_npz_root, 
        megadepth_train_list, 
        None, 
        'train', 
        cfg['img_resize'],
        cfg['seed'] + rank * 100, rank, world_size,
        score=0.0,
    ...
    for epoch in range(trained_epochs, cfg['epochs']):
        ...
        if rank == 0:
            path = save_weights(model,cfg["weights_dir"],'model',epoch,moving_mean_loss,warm_up_optimizer,warm_up_lr_scheduler)
    ...
    if rank == 0:
        path = save_weights(model,cfg["weights_dir"],'model',epoch+1,moving_mean_loss,using_optimizer,using_scheduler)
        print("save_weights to %s"%path,flush=True)
