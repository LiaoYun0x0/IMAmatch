...
# cfg['device'] = 'cpu:0'
cfg['batch_size'] = 1
cfg['log_file'] = 'log/eval_scannet.log'
cfg['thresh'] = 0.01
cfg['high_thresh'] = 2
cfg['allow_reverse'] = True
cfg['add_matches'] = 0
cfg['iter_optimize'] = True
cfg['rplus'] = 1.5           # 1 for megadepth, 1.5 for scannet
...
import models.matcher.attention as attn
attn.FIX_BUG_FOR_SCANNET = True
...
def eval(cfg):
    ...
    for n in model_state:
        if n in ckpts:
            recovery.append(n)
            model_state[n].copy_(ckpts[n])
        else:
            not_recovery.append(n)
    print('not recovery:',not_recovery)
    
    dataset_test = build_test_dataset(
        scannet_test_root_dir,
        scannet_test_npz_root,
        scannet_test_list,
        scannet_test_intrinsic_path,
        'val', 
        img_resize=None,
        score=0.0
    )
    ...
    for i,data in enumerate(tbar):        
        ...
        if not reverse:
            ...
            ret = estimate_pose(src_pts, dst_pts, data['K1'][0].cpu().numpy(), data['K0'][0].cpu().numpy(),thresh=0.5)
            if cfg['use_ba']:
                if ret is not None:
                    try:
                        inlier_mask = ret[2]
